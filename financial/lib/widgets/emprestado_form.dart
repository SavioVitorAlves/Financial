...
class EmprestadoForm extends StatefulWidget {
  final int id;
  const EmprestadoForm({required this.id, super.key});

  @override
  State<EmprestadoForm> createState() => _EmprestadoFormState();
}

class _EmprestadoFormState extends State<EmprestadoForm> {
  ...
  Future<void> _submitForm(BuildContext context) async {
    if (_descricaoControler.text.isEmpty) {
      showDialog(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: const Text(
              'Por favor, insira uma descrição ao dinheiro antes de adicionar.'),
          actions: [
            TextButton(
      ...
      showDialog(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: const Text(
              'Por favor, insira o valor do dinheiro antes de adicionar.'),
          actions: [
            TextButton(
                onPressed: () => Navigator.of(ctx).pop(),
                child: const Text('OK'))
    ...
    try {
      ...
      Navigator.of(context).pushNamed(AppRoutes.EMPRESTADO);
    } catch (error) {
      await showDialog(
          context: context,
          builder: (ctx) => AlertDialog(
